...
for (const kind of ['composition', 'match data']) {
  test(`${kind} reconnects after repeated browser fetch failures`, async t => {
    ...
    t.mock.method(globalThis, 'fetch', async () => {
      ...
      return new Response(JSON.stringify({ protocolVersion: '2.0', websocketUrl: 'wss://example.com/stream' }));
    });
    ...
    try {
      if (kind === 'composition') {
        ...
        await transport.open({ clientId: 'test', scopes: [], protocolVersions: ['2.0'],
