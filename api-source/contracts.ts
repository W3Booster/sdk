...
export type PointOfInterestKind = 'goblin-merchant' | 'marketplace' | 'mercenary-camp' |
  'goblin-laboratory' | 'tavern' | 'fountain' | 'gold-mine' | 'way-gate' |
  'dragon-roost' | 'goblin-shipyard' | 'creep-camp';
export interface PoiOffer {
  ...
  readonly typeId: string;
  readonly cost?: { readonly gold: number; readonly lumber: number; readonly food?: number };
  /** Omitted when unreadable. Zero means observed sold out. */
  readonly stock?: { readonly current: number; readonly max?: number };
...
export interface PointOfInterest {
  ...
  readonly wayGate?: { readonly enabled?: boolean; readonly destination?: Point; readonly destinationPoiId?: string };
  readonly camp?: {
    ...
    readonly dropTable?: readonly { readonly typeId?: string; readonly itemClass?: string; readonly level?: number; readonly probability?: number }[];
  };
}
...
export interface Building extends Unit {
  ...
  readonly upgrade?: TimedProgress & { readonly typeId: string };
...
export interface W3BoosterEventMap<TSettings extends object = JsonObject, TOverlayExtensions extends object = object> {
  ...
  'player.apm.changed': PlayerEvent<TSettings, TOverlayExtensions> & { readonly apm?: number; readonly previousApm?: number };
...
export declare function startClient<
  ...
  TOverlayExtensions extends object = object
>(
  options: ClientOptionsInput<TSettings, TOverlayExtensions>,
  startup?: StartupOptions
): Promise<W3BoosterClient<TSettings, TOverlayExtensions>>;

/** Create a client synchronously so lifecycle listeners can be attached before connecting. */
export declare function createClient<
  TSettings extends object = JsonObject,
  TOverlayExtensions extends object = object
>(options: ClientOptionsInput<TSettings, TOverlayExtensions>): W3BoosterClient<TSettings, TOverlayExtensions>;

export declare function isAbortError(error: unknown): error is AbortError;
export declare function isW3BoosterError(
  error: unknown
): error is PermissionRequiredError | ConnectionError | ProtocolError | HostActionError;
