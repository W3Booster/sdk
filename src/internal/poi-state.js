...
const point = value => shape(value, ['x', 'y']) && Number.isFinite(value.x) && Number.isFinite(value.y);
const shape = (value, keys) => isPlainObject(value) && Object.keys(value).every(key => keys.includes(key));
const optional = (value, key, valid) => value[key] === undefined || valid(value[key]);
const timer = value => shape(value, ['progress', 'remainingSeconds', 'totalSeconds']) && validTimedProgress(value);

export function validPoiOffer(value) {
  return shape(value, ['id', 'kind', 'typeId', 'cost', 'stock', 'initialAvailability', 'restock', 'cooldown', 'stockPoolId', 'eligibility']) &&
    identity(value.id) && ['item', 'unit', 'hero', 'service'].includes(value.kind) && isTypeId(value.typeId) &&
    optional(value, 'cost', cost => shape(cost, ['gold', 'lumber', 'food']) && number(cost.gold) && number(cost.lumber) && optional(cost, 'food', number)) &&
    optional(value, 'stock', stock => shape(stock, ['current', 'max']) && integer(stock.current) && optional(stock, 'max', max => integer(max) && stock.current <= max)) &&
    ...
    optional(value, 'stockPoolId', identity) && optional(value, 'eligibility', entries => isPlainObject(entries) &&
      Object.entries(entries).every(([key, entry]) => identity(key) && shape(entry, ['available', 'reasons']) &&
...
export function validPointOfInterest(value) {
  if (!shape(value, ['id', 'kind', 'typeId', 'position', 'observedAtGameTime', 'guardCampId', 'offers', 'nextStockUpdate', 'fountain', 'mine', 'wayGate', 'camp']) ||
    !identity(value.id) || !KINDS.has(value.kind) || !point(value.position) || !number(value.observedAtGameTime) ||
    !optional(value, 'typeId', isTypeId) || !optional(value, 'guardCampId', identity) ||
  ...
  if (value.fountain !== undefined) {
    ...
    if (value.kind !== 'fountain' || !shape(f, ['restores', 'radius', 'active', 'healthPerSecond', 'manaPerSecond', 'healthFractionPerSecond', 'manaFractionPerSecond']) ||
  ...
  if (value.mine !== undefined) {
    ...
    if (value.kind !== 'gold-mine' || !shape(mine, ['remainingGold', 'initialGold', 'ownerPlayerId', 'buildingId']) ||
  ...
  if (value.wayGate !== undefined) {
    ...
    if (value.kind !== 'way-gate' || !shape(gate, ['enabled', 'destination', 'destinationPoiId']) ||
  ...
  if (value.camp !== undefined) {
    ...
    if (value.kind !== 'creep-camp' || !shape(camp, ['state', 'members', 'dropTable']) ||
      !['alive', 'partially-cleared', 'cleared', 'unknown'].includes(camp.state) || !Array.isArray(camp.members) || camp.members.length > 8192 ||
      !camp.members.every(member => shape(member, ['id', 'typeId', 'alive', 'position', 'hitpoints']) && isInstanceId(member.id) && isTypeId(member.typeId) &&
    ...
    if (camp.dropTable !== undefined && (!Array.isArray(camp.dropTable) || camp.dropTable.length > 256 || !camp.dropTable.every(drop =>
      shape(drop, ['typeId', 'itemClass', 'level', 'probability']) && (drop.typeId !== undefined || drop.itemClass !== undefined) &&
